...
namespace NAudioLibrary
{
    ...
    public abstract class SpeexChatCodec : INetworkChatCodec
    {
        ...
        public byte[] Encode(byte[] data, int offset, int length)
        {
            ...
            Debug.WriteLine( $"NSpeex: In {length} bytes, encoded {bytesWritten} bytes [enc frame size = {encoder.FrameSize}]");
            return encoded;
        }
        ...
        public byte[] Decode(byte[] data, int offset, int length)
        {
            ...
            //    lost = true;
            //}
            int samplesDecoded = decoder.Decode(data, offset, length, wb.ShortBuffer, 0, lost);
            int bytesDecoded = samplesDecoded * 2;
            var decoded = new byte[bytesDecoded];
            Array.Copy(outputBufferTemp, 0, decoded, 0, bytesDecoded);
            Debug.WriteLine(  $"NSpeex: In {length} bytes, decoded {bytesDecoded} bytes [dec frame size = {decoder.FrameSize}]");
            return decoded;
        }
